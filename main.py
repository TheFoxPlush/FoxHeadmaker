...
from textual.widgets import Header, Input, Collapsible, Link, Button, DirectoryTree, Static, LoadingIndicator, ProgressBar, Checkbox, Label
from textual.containers import HorizontalGroup, VerticalScroll, Center, Container, Horizontal
...
def clear_cache(folder):
    total_size = 0
    total_files = 0
    for cleared_file in os.listdir(folder):
        total_files+=1
        total_size+=os.path.getsize(os.path.join(folder,cleared_file))
        os.remove(os.path.join(folder,cleared_file))
    
    total_size = f"{total_size/(1<<17):,.0f}"
    return(total_files,total_size)

def clear_head_cache():
    total_files,total_size = clear_cache(CACHE_HEADS_DIR)
    app.notify(f"Cleared {total_files} head files, saving {total_size} Mb", title="Head Cache Cleared")
    app.reset_head_compilation()

def clear_value_cache():
    total_files,total_size = clear_cache(CACHE_VALUES_DIR)
    app.notify(f"Cleared {total_files} value files, saving {total_size} Mb", title="Value Cache Cleared")
    app.reset_mineskin_process()
...
class UpdateScreen(ModalScreen):
    ...
    UpdateScreen > Container > Label {
        width: 100%;
        content-align-horizontal: center;
        margin-top: 1;
    }

    UpdateScreen > Container > Horizontal {
        width: auto;
        height: auto;
    }

    UpdateScreen > Container > Horizontal > Button {
        margin: 2 4;
    }
    """
...
class FoxHeadmakerApp(App):
    ...
    def get_system_commands(self, screen: Screen):
        ...
        yield SystemCommand("Clear Head Cache","Clears the images generated by head compilation.",clear_head_cache)
        yield SystemCommand("Clear Value Cache","Clears the values obtained from mineskin.org.",clear_value_cache)
    ...
    def reset_mineskin_process(self) -> None:
        self.query_one("#loadingbar_headtexture",Static).update(f"[underline]🎨 Head texture[/]\n")
        self.query_one("#loadingbar_mineskin",Static).update(f"[underline]💻 mineskin.org[/]\n")
        self.query_one("#loadingbar_skinvalue",Static).update(f"[underline]💾 Skin Value[/]\n")
    ...
    def do_update(self, accept: bool):
        if accept:
